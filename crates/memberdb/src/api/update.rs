...
pub async fn add_member_discord(
    tx: &mut Transaction<'_>, discord_id: DiscordId, rank: MemberRank,
) -> Result<MemberId> {
    ...
    tx.db.signal(DBEvent::MemberAdd {
        mid,
...
pub async fn add_member_wynn(
    tx: &mut Transaction<'_>, mcid: &str, rank: MemberRank, ign: &str,
) -> Result<MemberId> {
...
pub async fn add_member(
    tx: &mut Transaction<'_>, discord_id: DiscordId, mcid: &str, ign: &str, rank: MemberRank,
) -> Result<MemberId> {
...
async fn to_full_member(tx: &mut Transaction<'_>, mid: MemberId) -> Result<()> {
...
async fn to_guild_partial(tx: &mut Transaction<'_>, mid: MemberId) -> Result<()> {
...
async fn add_discord_profile(
    tx: &mut Transaction<'_>, mid: Option<MemberId>, discord_id: DiscordId,
) -> Result<()> {
    ...
    tx.db.signal(DBEvent::DiscordProfileAdd { discord_id, mid });
    Ok(())
}
...
async fn add_wynn_profile(
    tx: &mut Transaction<'_>, mid: Option<MemberId>, mcid: &str, ign: &str,
) -> Result<()> {
    ...
    tx.db.signal.signal(DBEvent::WynnProfileAdd { mcid: mcid.to_string(), mid });
    Ok(())
}
...
async fn add_guild_profile(tx: &mut Transaction<'_>, mcid: &str, rank: GuildRank) -> Result<()> {
    ...
    tx.db.signal(DBEvent::GuildProfileAdd { mcid: mcid.to_string(), rank });
    Ok(())
}

/// Update a member's rank.
/// Note that this function won't broadcast the `MemberRankChange` event.
pub async fn update_member_rank(tx: &mut Transaction<'_>, mid: MemberId, rank: MemberRank) -> Result<()> {
...
pub async fn update_message(tx: &mut Transaction<'_>, amount: i64, discord_id: DiscordId) -> Result<()> {
    query!(
...
pub async fn update_voice(tx: &mut Transaction<'_>, amount: i64, discord_id: DiscordId) -> Result<()> {
...
pub async fn update_activity(tx: &mut Transaction<'_>, mcid: &str, amount: i64) -> Result<()> {
    query!(
...
pub async fn update_ign(tx: &mut Transaction<'_>, mcid: &str, ign: &str) -> Result<()> {
...
pub async fn update_guild_rank(tx: &mut Transaction<'_>, mcid: &str, rank: GuildRank) -> Result<()> {
...
pub async fn update_xp(tx: &mut Transaction<'_>, mcid: &str, amount: i64) -> Result<()> {
...
/// Update a member's discord link, and return true if the member is removed.
///
/// # Preconditions
/// The new discord profile is unlinked.
#[instrument(skip(tx))]
pub async fn bind_discord(
    tx: &mut Transaction<'_>, mid: MemberId, discord_new: Option<DiscordId>,
) -> Result<bool> {
    let discord_old = query!("SELECT discord FROM member where oid=?", mid)
        .fetch_one(&tx.db.pool)
        .await
    ...
    let has_removed = {
        if discord_new.is_none() {
            ...
            let (_, mcid) = get_member_links(&tx.db, mid).await?;
            match mcid {
                Some(mcid) => {
                    if is_in_guild(&tx.db, &mcid).await? {
                        info!("Member is in guild, demote to guild partial");
                        let before = get_member_type(&tx.db, mid).await?;
                        to_guild_partial(tx, mid).await?;
                        tx.db.signal(DBEvent::MemberAutoGuildDemote { mid, before });
                        false
                    } else {
                        ...
                        tx.db.signal(DBEvent::WynnProfileUnbind {
                            mid,
                            before: mcid.to_string(),
                            removed: true,
                        });
                        tx.db.signal(DBEvent::MemberRemove {
                            mid,
                ...
                None => {
                    ...
                    remove_member_unchecked(tx, mid).await?;
                    tx.db.signal(DBEvent::MemberRemove { mid, discord_id: discord_old, mcid: None });
                    true
                }
            }
        } else {
            info!(mid, "Checking if member should be promoted");
            match get_member_type(tx.db, mid).await? {
                before @ MemberType::GuildPartial | before @ MemberType::WynnPartial => {
                    to_full_member(tx, mid).await?;
                    tx.db.signal(DBEvent::MemberFullPromote { mid, before });
                }
                _ => {}
            }
            false
    ...
    tx.db.signal(match discord_new {
...
pub async fn bind_wynn(
    tx: &mut Transaction<'_>, mid: MemberId, mcid_new: Option<&str>, ign: &str,
) -> Result<bool> {
    ...
    if let MemberType::WynnPartial = get_member_type(tx.db, mid).await? {
        if let Some(mcid_old) = &mcid_old {
            if mcid_new.is_none() && is_in_guild(tx.db, mcid_old).await? {
                info!("Removing wynn binding on wynn partial, but player is in guild, so updated to guild partial");
                to_guild_partial(tx, mid).await?;
                tx.db
                    .signal(DBEvent::MemberAutoGuildDemote { mid, before: MemberType::WynnPartial });
                return Ok(false);
            }
        }
    }
    ...
    let has_removed = if mcid_new.is_none() {
        ...
        let (discord_id, _) = get_member_links(tx.db, mid).await?;
        ...
        match get_member_type(tx.db, mid).await? {
            MemberType::DiscordPartial => {
                ...
                to_full_member(tx, mid).await?;
                tx.db
                    .signal(DBEvent::MemberFullPromote { mid, before: MemberType::DiscordPartial });
            }
            _ => {}
        }
        false
...
/// # Preconditions
/// The new wynn profile is unlinked.
async fn bind_wynn_unchecked(tx: &mut Transaction<'_>, mid: MemberId, mcid_new: Option<&str>) -> Result<()> {
...
pub async fn bind_wynn_guild(
    tx: &mut Transaction<'_>, mcid: &McId, ign: &str, status: bool, rank: GuildRank,
) -> Result<Option<MemberId>> {
    if !wynn_profile_exist(tx.db, mcid).await? {
        info!("Adding missing wynn profile");
        add_wynn_profile(tx, None, mcid, ign).await?;
    }
    if !guild_profile_exist(tx.db, mcid).await? {
    ...
    match get_wynn_mid(tx.db, &mcid).await? {
        Some(mid) => {
            ...
            if !status {
                if let MemberType::GuildPartial = get_member_type(tx.db, mid).await? {
                    ...
                    let (discord, _) = get_member_links(tx.db, mid).await?;
                    remove_member_unchecked(tx, mid).await?;
                    tx.db.signal(DBEvent::MemberRemove {
                        mid,
                        discord_id: discord,
        ...
        None => {
            ...
            if status {
                ...
                tx.db.signal(DBEvent::WynnProfileBind { mid, old: None, new: mcid.to_string() });
                tx.db.signal(DBEvent::MemberAdd {
                    mid,
                    discord_id: None,
                    mcid: Some(mcid.to_string()),
...
async fn link_discord(tx: &mut Transaction<'_>, mid: Option<MemberId>, discord: DiscordId) -> Result<()> {
...
/// This function doesn't ensure database integrity.
#[instrument(skip(tx))]
async fn link_or_add_discord(
    tx: &mut Transaction<'_>, mid: Option<MemberId>, discord: DiscordId,
) -> Result<()> {
    if discord_profile_exist(tx.db, discord).await? {
...
/// This function doesn't ensure database integrity.
async fn link_wynn(tx: &mut Transaction<'_>, mid: Option<MemberId>, mcid: &str) -> Result<()> {
...
async fn remove_member_unchecked(tx: &mut Transaction<'_>, mid: MemberId) -> Result<()> {
...
#[instrument(skip(tx))]
pub async fn remove_member(tx: &mut Transaction<'_>, mid: MemberId) -> Result<()> {
    let (discord, mcid) = get_member_links(tx.db, mid).await?;
    ...
    if mcid.is_some() {
        ...
        if bind_wynn(tx, mid, None, "").await? {
            tx.db.signal(DBEvent::MemberRemove { mid, discord_id: discord, mcid });
            return Ok(());
        }
    }
